...
export class FlaggedService {
    ...
    GetFlagged$ = this.GetFlaggedAction.pipe(
        switchMap(() => {
            return this.http.get('http://localhost:8080/getFlagged', {responseType: "text"}).pipe(
                tap(res => {
    ...
    setFlagged(form: any) {
        return this.http.post('http://localhost:8080/setFlagged', form.controls.flaggedPlayers.value, {responseType: "text"}).pipe(
            tap((res: any) => {
