...
export class DisplayedColumnsService {
    ...
    getDisplayedColumns$ = this.GetDisplayedColumnsAction.pipe(
        switchMap((type) => {
            ...
            return this.http.get('http://localhost:8080/' + endpoint, {responseType: "text"}).pipe(
                tap(res => {
    ...
    setColumns(columns: any, type: string) {
        ...
        return this.http.post('http://localhost:8080/' + endpoint, columns, {responseType: 'text'}).pipe(
            tap((res: any) => {
                console.log(res);
